...
def main() -> None:
    ...
    # input folder (project_root/data/raw) and default output folder (project_root/data)
    input_folder = project_root / "data" / "raw"
    input_folder.mkdir(parents=True, exist_ok=True)
    default_out_folder = project_root / "data"
    default_out_folder.mkdir(parents=True, exist_ok=True)

    # decide input CSV
    if args.input is None:
        csvs = sorted(input_folder.glob("*.csv"))
        if not csvs:
            raise FileNotFoundError(f"No CSV files found in {input_folder}. Provide --input to specify the file.")
        input_path = csvs[0]
